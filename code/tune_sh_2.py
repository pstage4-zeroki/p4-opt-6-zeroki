...
def search_model(trial: optuna.trial.Trial) -> List[Any]:
    ...
    if m2 == "Conv":
        ...
        m2_c = trial.suggest_int("m2/mix_c", low=global_output_channel, high=40, step=8)
        m2_max_kernel = 3 # trial.suggest_int("m2/mix_k", low=3, high=5)
        m2_kernels = list(range(3, m2_max_kernel+1, 2))  # make kernel list [3, 5, ... , max_kernel]
        m2_t = trial.suggest_int("m2/mix_t", low=1, high=3)
        m2_se = trial.suggest_categorical("m2/mix_se", [0, 1])
        m2_activation = trial.suggest_categorical("m2/activation", ["ReLU", "Swish"])
        m2_args = [m2_kernels, m2_t, m2_c, m2_se, m2_stride, m2_activation]
        m2_sub_args = [m2_kernels, m2_t, m2_c, m2_se, 1, m2_activation]
    ...
    if m3 == "Conv":
        ...
        m3_c = global_output_channel*2
        m3_args = [m3_stride]
        m3_sub_args = [1]
    elif m3 == "MixNetBlock":
        m3_c = trial.suggest_int("m3/mix_c", low=global_output_channel, high=96, step=8)
        m3_max_kernel = trial.suggest_int("m3/mix_k", low=5, high=7, step=2)
        m3_kernels = list(range(3, m3_max_kernel+1, 2))  # make kernel list [3, 5, ... , max_kernel]
        m3_t = trial.suggest_int("m3/mix_t", low=1, high=3)
        m3_se = trial.suggest_categorical("m3/mix_se", [0, 1])
        m3_activation = trial.suggest_categorical("m3/activation", ["ReLU", "Swish"])
        m3_args = [m3_kernels, m3_t, m3_c, m3_se, m3_stride, m3_activation]
        m3_sub_args = [m3_kernels[:-1], m3_t, m3_c, m3_se, 1, m3_activation]
    ...
    if m4 == "Conv":
        ...
        m4_c = trial.suggest_int("m4/mix_c", low=global_output_channel, high=128, step=8)
        m4_max_kernel = trial.suggest_int("m4/mix_k", low=5, high=7, step=2)
        m4_kernels = list(range(3, m4_max_kernel+1, 2))  # make kernel list [3, 5, ... , max_kernel]
        m4_t = trial.suggest_int("m4/mix_t", low=1, high=3)
        m4_se = trial.suggest_categorical("m4/mix_se", [0, 1])
        m4_activation = trial.suggest_categorical("m4/activation", ["ReLU", "Swish"])
        m4_args = [m4_kernels, m4_t, m4_c, m4_se, m4_stride, m4_activation]
        m4_sub_args = [m4_kernels[:-1], m4_t, m4_c, m4_se, 1, m4_activation]
    ...
    if m5 == "Conv":
        ...
        m5_c = trial.suggest_int("m5/mix_c", low=global_output_channel, high=256, step=8)
        m5_max_kernel = trial.suggest_int("m5/mix_k", low=5, high=9, step=2)
        m5_kernels = list(range(3, m5_max_kernel+1, 2))  # make kernel list [3, 5, ... , max_kernel]
        m5_t = trial.suggest_int("m5/mix_t", low=1, high=3)
        m5_se = trial.suggest_categorical("m5/mix_se", [0, 1])
        m5_activation = trial.suggest_categorical("m5/activation", ["ReLU", "Swish"])
        m5_args = [m5_kernels, m5_t, m5_c, m5_se, m5_stride, m5_activation]
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    run = wandb.init(entity="zeroki",
                    project='CUCU_CUCHEN', #'CUCU_CUCHEN',
                    group="Real_GAUZA",
                    name=f'Trial_{trial.number}',
                    config=model_config,
                    reinit=True,
                    save_code=True  ##### 돌린 코드 저장!!
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--study-name", default="Real_GAUZA", type=str, help="Optuna study name.")
