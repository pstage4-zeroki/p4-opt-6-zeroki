...
def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    ...
    epochs = 70     #trial.suggest_int("epochs", low=50, high=200, step=50)
...
def search_model(trial: optuna.trial.Trial) -> List[Any]:
    ...
    m2_args = []
    m2_out_channel = trial.suggest_int("m2/out_channels", low=16, high=64, step=16)
    m2_stride = trial.suggest_int("m2/stride", low=1, high=UPPER_STRIDE)

    # force stride m2
    if n_stride == 0:
        m2_stride = 2
    # no repeat when stride is 2
    if m2_stride == 2:
        m2_repeat = 1
    else:
    ...
    if m2 == "Conv":
        ...
        m2_t = trial.suggest_int("m2/MB_t", low=1, high=4)
        m2_c = trial.suggest_int("m2/MB_c", low=16, high=32, step=8)
        m2_kernel = trial.suggest_int("m2/kernel_size", low=3, high=5, step=2)
        m2_args = [m2_t, m2_c, m2_stride, m2_kernel]
    elif m2 == "ShuffleNetV2":
        m2_c = trial.suggest_int("m2/shuffle_c", low=16, high=32, step=8)
        m2_args = [m2_stride]
    if not m2 == "Pass":
        if m2_stride == 2:
            n_stride += 1
    ...
    if m3 == "Conv":
        # Conv args: [out_channel, kernel_size, stride, padding, groups, activation]
        m3_out_channel = trial.suggest_int("m3/out_channels", low=16, high=64, step=16)
        ...
        m3_t = trial.suggest_int("m3/MB_t", low=1, high=4)
        m3_c = trial.suggest_int("m3/MB_c", low=16, high=32, step=8)
        m3_kernel = trial.suggest_int("m3/kernel_size", low=3, high=5, step=2)
        m3_args = [m3_t, m3_c, m3_stride, m3_kernel]
    elif m3 == "ShuffleNetV2":
        m3_c = trial.suggest_int("m3/shuffle_c", low=16, high=32, step=8)
        m3_args = [m3_stride]
    if not m3 == "Pass":
        if m3_stride == 2:
            n_stride += 1
    ...
    if m4 == "Conv":
        ...
        m4_t = trial.suggest_int("m4/MB_t", low=1, high=4)
        m4_c = trial.suggest_int("m4/MB_c", low=16, high=32, step=8)
        m4_kernel = trial.suggest_int("m4/kernel_size", low=3, high=5, step=2)
        m4_args = [m4_t, m4_c, m4_stride, m4_kernel]
    elif m4 == "ShuffleNetV2":
        m4_c = trial.suggest_int("m4shuffle_c", low=16, high=32, step=8)
        m4_args = [m4_stride]
    if not m4 == "Pass":
        if m4_stride == 2:
            n_stride += 1
    ...
    if m5 == "Conv":
        ...
        m5_t = trial.suggest_int("m5/MB_t", low=1, high=4)
        m5_c = trial.suggest_int("m5/MB_c", low=16, high=32, step=8)
        m5_kernel = trial.suggest_int("m5/kernel_size", low=3, high=5, step=2)
    ...
    if m6 == "Conv":
        ...
        m6_t = trial.suggest_int("m6/MB_t", low=1, high=4)
        m6_c = trial.suggest_int("m6/MB_c", low=16, high=32, step=8)
        m6_kernel = trial.suggest_int("m6/kernel_size", low=3, high=5, step=2)
        m6_args = [m6_t, m6_c, m6_stride, m6_kernel]
    elif m6 == "ShuffleNetV2":
        m6_c = trial.suggest_int("m6/shuffle_c", low=16, high=32, step=8)
        m6_args = [m6_stride]
    if not m6 == "Pass":
        if m6_stride == 2:
            n_stride += 1
    ...
    if m7_stride == 2:
        ...
        m7_repeat = trial.suggest_int("m6/repeat", 1, 5)

    if m7 == "Conv":
        # Conv args: [out_channel, kernel_size, stride, padding, groups, activation]
        m7_out_channel = trial.suggest_int("m7/out_channels", low=128, high=512, step=128)
        m7_kernel = trial.suggest_int("m7/kernel_size", low=1, high=3, step=2)
        ...
        m7_t = trial.suggest_int("m7/MB_t", low=1, high=4)
        m7_c = trial.suggest_int("m7/MB_c", low=16, high=32, step=8)
        m7_kernel = trial.suggest_int("m7/kernel_size", low=3, high=5, step=2)
        m7_args = [m7_t, m7_c, m7_stride, m7_kernel]
    elif m7 == "ShuffleNetV2":
        m7_c = trial.suggest_int("m7/shuffle_c", low=16, high=32, step=8)
        m7_args = [m7_stride]
    if not m7 == "Pass":
        if m7_stride == 2:
            n_stride += 1
    ...
    # last layer
    last_dim = trial.suggest_int("last_dim", low=128, high=256, step=128)
...
def tune(gpu_id: int, storage: Union[str, None] = None, study_name: str = "pstage_automl"):
    ...
    study = optuna.create_study(
        ...
        storage="postgresql://my_optuna:1q2w3e4r@118.67.134.251:6011/optuna",
